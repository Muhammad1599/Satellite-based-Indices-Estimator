...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bsi_analysis.log'),
...
def fill_gaps(df: pd.DataFrame, max_gap_days: int = 32) -> pd.DataFrame:
    """
    Fill temporal gaps in BSI time series using a combination of
    Whittaker smoothing and spline interpolation.
    
    Args:
        df: Input DataFrame with date and BSI columns
    ...
    gaps = merged_df['BSI_mean'].isnull()
    ...
    # Apply Whittaker smoothing to mean BSI
    x = merged_df['BSI_mean'].fillna(merged_df['BSI_mean'].mean()).values
    smoothed_bsi = whittaker_smooth(x, weights)
    
    # Use spline interpolation for final adjustments
    valid_idx = ~merged_df['BSI_mean'].isnull()
    if valid_idx.sum() > 3:  # Need at least 4 points for cubic spline
        spline = CubicSpline(
            np.arange(len(merged_df))[valid_idx],
            merged_df['BSI_mean'][valid_idx],
            bc_type='natural'
        )
        # Blend smoothed and spline results
        alpha = 0.7  # Weight for smoothed values
        merged_df.loc[merged_df['BSI_mean'].isnull(), 'BSI_mean'] = \
            alpha * smoothed_bsi[merged_df['BSI_mean'].isnull()] + \
            (1 - alpha) * spline(np.arange(len(merged_df))[merged_df['BSI_mean'].isnull()])
    else:
        # If too few points, use only Whittaker smoothing
        merged_df.loc[merged_df['BSI_mean'].isnull(), 'BSI_mean'] = \
            smoothed_bsi[merged_df['BSI_mean'].isnull()]
    
    # Interpolate other columns
    for col in ['BSI_min', 'BSI_max', 'BSI_stdDev']:
...
class BSIAnalyzer:
    """Class to handle BSI analysis operations"""
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        default_config = {
            ...
            'chunk_size_days': 15,
            'scale_meters': 20,  # Changed to 20m as SWIR band (B11) is 20m resolution
            'max_pixels': 1e9,
    ...
    def calculate_bsi(self, image):
        """Calculate BSI for a single image"""
        try:
            # Select bands and normalize to 0-1 scale (Sentinel-2 data is in 0-10000 scale)
            SWIR = image.select('B11').divide(10000)  # SWIR band
            ...
            numerator = SWIR.add(RED).subtract(NIR).subtract(BLUE)
            denominator = SWIR.add(RED).add(NIR).add(BLUE)
            bsi = numerator.divide(denominator)
            
            return bsi.rename('BSI')
        except Exception as e:
            logging.error(f"Error calculating BSI: {e}")
            raise
        ...
        try:
            ...
            def process_image(image):
                ...
                return ee.Feature(None, {
                    ...
                    'BSI_stdDev': stats.get('BSI_stdDev'),
                    'BSI_min': stats.get('BSI_min'),
                    'BSI_max': stats.get('BSI_max'),
            ...
            for feature in features:
                if feature['properties']['BSI_mean'] is not None:
    ...
    def plot_time_series(self, df: pd.DataFrame, output_file: str):
        ...
        if 'is_interpolated' in df.columns:
            ...
            # Plot actual data
            ax1.plot(actual['date'], actual['BSI_mean'], 'b-', 
                    label='Mean BSI (Observed)', linewidth=2, alpha=0.7)
            ax1.scatter(actual['date'], actual['BSI_mean'], 
                       color='blue', s=100, zorder=5, label='Observation')
            
            # Plot interpolated data
            if not interp.empty:
                ax1.plot(interp['date'], interp['BSI_mean'], 'b--', 
                        label='Mean BSI (Interpolated)', linewidth=1, alpha=0.5)
                ax1.scatter(interp['date'], interp['BSI_mean'], 
                          color='lightblue', s=50, zorder=4, 
                          label='Interpolated', alpha=0.5)
        else:
            # Original plotting code for non-interpolated data
            ax1.plot(df['date'], df['BSI_mean'], 'b-', 
                    label='Mean BSI', linewidth=2, alpha=0.7)
            ax1.scatter(df['date'], df['BSI_mean'], 
                       color='blue', s=100, zorder=5, label='Observation')
        
        # Add error bands
        ax1.fill_between(df['date'], 
                        df['BSI_mean'] - df['BSI_stdDev'],
                        df['BSI_mean'] + df['BSI_stdDev'],
                        alpha=0.2, color='blue', label='±1 StdDev')
        
        # Add min/max as points with connecting lines
        ax1.plot(df['date'], df['BSI_min'], 'r--', alpha=0.5, linewidth=1, label='Min BSI')
        ax1.plot(df['date'], df['BSI_max'], 'g--', alpha=0.5, linewidth=1, label='Max BSI')
        ax1.scatter(df['date'], df['BSI_min'], color='red', s=50, alpha=0.5, zorder=4)
        ax1.scatter(df['date'], df['BSI_max'], color='green', s=50, alpha=0.5, zorder=4)
        ...
        ax1.set_title('BSI Time Series Analysis', pad=20, fontsize=14)
        ax1.set_ylabel('BSI Value', fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        
        # Set y-axis limits with some padding
        ymin = df['BSI_min'].min() - 0.1
        ymax = df['BSI_max'].max() + 0.1
    ...
    def validate_results(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        # Check for unrealistic BSI values (typically between -1 and 1)
        if (df['BSI_mean'] > 1).any() or (df['BSI_mean'] < -1).any():
            issues.append("Unrealistic BSI values detected")
    ...
    def run_analysis(self, geojson_path: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            csv_file = self.output_dir / f'bsi_results_{timestamp}.csv'
            plot_file = self.output_dir / f'bsi_timeseries_{timestamp}.png'
            ...
            metadata = {
                ...
                'mean_bsi': float(df_filled['BSI_mean'].mean()) if not df_filled.empty else None,
