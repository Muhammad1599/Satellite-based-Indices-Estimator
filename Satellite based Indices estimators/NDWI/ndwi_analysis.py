...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ndwi_analysis.log'),
...
def fill_gaps(df: pd.DataFrame, max_gap_days: int = 32) -> pd.DataFrame:
    """
    Fill temporal gaps in NDWI time series using a combination of
    Whittaker smoothing and spline interpolation.
    
    Args:
        df: Input DataFrame with date and NDWI columns
    ...
    gaps = merged_df['NDWI_mean'].isnull()
    ...
    # Apply Whittaker smoothing to mean NDWI
    x = merged_df['NDWI_mean'].fillna(merged_df['NDWI_mean'].mean()).values
    smoothed_ndwi = whittaker_smooth(x, weights)
    
    # Use spline interpolation for final adjustments
    valid_idx = ~merged_df['NDWI_mean'].isnull()
    if valid_idx.sum() > 3:  # Need at least 4 points for cubic spline
        spline = CubicSpline(
            np.arange(len(merged_df))[valid_idx],
            merged_df['NDWI_mean'][valid_idx],
            bc_type='natural'
        )
        # Blend smoothed and spline results
        alpha = 0.7  # Weight for smoothed values
        merged_df.loc[merged_df['NDWI_mean'].isnull(), 'NDWI_mean'] = \
            alpha * smoothed_ndwi[merged_df['NDWI_mean'].isnull()] + \
            (1 - alpha) * spline(np.arange(len(merged_df))[merged_df['NDWI_mean'].isnull()])
    else:
        # If too few points, use only Whittaker smoothing
        merged_df.loc[merged_df['NDWI_mean'].isnull(), 'NDWI_mean'] = \
            smoothed_ndwi[merged_df['NDWI_mean'].isnull()]
    
    # Interpolate other columns
    for col in ['NDWI_min', 'NDWI_max', 'NDWI_stdDev']:
...
class NDWIAnalyzer:
    """Class to handle NDWI analysis operations"""
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        default_config = {
            ...
            'chunk_size_days': 15,
            'scale_meters': 10,  # Changed to 10m as both Green and NIR are 10m resolution
            'max_pixels': 1e9,
    ...
    def process_date_chunk(self, geometry: ee.Geometry, start_date: str, end_date: str) -> List[Dict]:
        ...
        try:
            ...
            for feature in features:
                if feature['properties']['NDWI_mean'] is not None:
    ...
    def plot_time_series(self, df: pd.DataFrame, output_file: str):
        ...
        if 'is_interpolated' in df.columns:
            ...
            # Plot actual data
            ax1.plot(actual['date'], actual['NDWI_mean'], 'b-', 
                    label='Mean NDWI (Observed)', linewidth=2, alpha=0.7)
            ax1.scatter(actual['date'], actual['NDWI_mean'], 
                       color='blue', s=100, zorder=5, label='Observation')
            
            # Plot interpolated data
            if not interp.empty:
                ax1.plot(interp['date'], interp['NDWI_mean'], 'b--', 
                        label='Mean NDWI (Interpolated)', linewidth=1, alpha=0.5)
                ax1.scatter(interp['date'], interp['NDWI_mean'], 
                          color='lightblue', s=50, zorder=4, 
                          label='Interpolated', alpha=0.5)
        else:
            # Original plotting code for non-interpolated data
            ax1.plot(df['date'], df['NDWI_mean'], 'b-', 
                    label='Mean NDWI', linewidth=2, alpha=0.7)
            ax1.scatter(df['date'], df['NDWI_mean'], 
                       color='blue', s=100, zorder=5, label='Observation')
        
        # Add error bands
        ax1.fill_between(df['date'], 
                        df['NDWI_mean'] - df['NDWI_stdDev'],
                        df['NDWI_mean'] + df['NDWI_stdDev'],
                        alpha=0.2, color='blue', label='±1 StdDev')
        
        # Add min/max as points with connecting lines
        ax1.plot(df['date'], df['NDWI_min'], 'r--', alpha=0.5, linewidth=1, label='Min NDWI')
        ax1.plot(df['date'], df['NDWI_max'], 'g--', alpha=0.5, linewidth=1, label='Max NDWI')
        ax1.scatter(df['date'], df['NDWI_min'], color='red', s=50, alpha=0.5, zorder=4)
        ax1.scatter(df['date'], df['NDWI_max'], color='green', s=50, alpha=0.5, zorder=4)
        ...
        ax1.set_title('NDWI Time Series Analysis', pad=20, fontsize=14)
        ax1.set_ylabel('NDWI Value', fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.legend(loc='center left', bbox_to_anchor=(1, 0.5))
        
        # Set y-axis limits with some padding
        ymin = df['NDWI_min'].min() - 0.1
        ymax = df['NDWI_max'].max() + 0.1
    ...
    def validate_results(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        # Check for unrealistic NDWI values (typically between -1 and 1)
        if (df['NDWI_mean'] > 1).any() or (df['NDWI_mean'] < -1).any():
            issues.append("Unrealistic NDWI values detected")
    ...
    def run_analysis(self, geojson_path: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            csv_file = self.output_dir / f'ndwi_results_{timestamp}.csv'
            plot_file = self.output_dir / f'ndwi_timeseries_{timestamp}.png'
            ...
            metadata = {
                ...
                'mean_ndwi': float(df_filled['NDWI_mean'].mean()) if not df_filled.empty else None,
